...
contract IdleCDO is PausableUpgradeable, GuardedLaunchUpgradable, IdleCDOStoragePolygon {
    ...
    // Pass values as params rather than hard coding
    constructor(
        address _stkAave,
        address _aave,
        address _weth,
    ...
    ) IdleCDOStoragePolygon(_stkAave, _aave, _weth, _uniswapV2Router02, _feeReceiver) {
        token = address(1);
    }
    ...
    function getContractValue() public view override returns (uint256) {
        ...
        return
            ((_contractTokenBalance(_strategyToken) * _strategyPrice()) / (10**(strategyTokenDecimals))) +
    ...
    function _sellReward(
        ...
        uint256[] memory _amounts = _uniRouter.swapExactTokensForTokens(_amount, _minAmount, _path, address(this), block.timestamp + 1);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _rewards.length; i++) {
            ...
            if (_skipReward[i] || _includesAddress(_incentiveTokens, _rewardToken)) {
                continue;
            }
