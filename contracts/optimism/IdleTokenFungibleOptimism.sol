...
// Changed only TL_MULTISIG, DL_MULTISIG, PAUSE_MULTISIG and fee = 10000 wrt the mainnet version

contract IdleTokenFungibleOptimism is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  address public constant DL_MULTISIG = 0xFDbB4d606C199F091143BD604C85c191a526fbd0;
  address public constant PAUSE_MULTISIG = 0xFDbB4d606C199F091143BD604C85c191a526fbd0;
  bool public skipRedeemMinAmount;
  uint256 public scaleForTolerance;
  bool public redeemWithQueue;
  ...
  /**
   * It allows owner to set allow proportional redeem or redeem in queue
   *
   * @param _flag : wheter to activate redeem with queue or not
   */
  function setRedeemWithQueue(bool _flag)
    external onlyOwner {
      redeemWithQueue = _flag;
  }
  ...
  */
    ...
    returns (uint256 redeemedTokens) {
      ...
      if (_amount != 0) {
        ...
        if (valueToRedeem > balanceUnderlying) {
          if (redeemWithQueue) {
            // we pass valueToRedeem of underlyings and redeem from each protocol one after the other until
            // we reach the desired amount
            redeemedTokens = _redeemHelper(valueToRedeem - balanceUnderlying) + balanceUnderlying;
          } else {
            // we pass _amount of idleToken and redeem proportionally from each protocol + from the unlent balance
            redeemedTokens = _redeemHelperProportional(_amount, balanceUnderlying);
          }
        } else {
  ...
  /**
   * Here we calc the pool share one can withdraw given the amount of IdleToken they want to burn
   * and redeem proportionally from each integrated protocol + from the unlent balance
   *
   * @param _amount : amount in underlyings
   * @return redeemedTokens : amount of underlying tokens redeemed
   */
  function _redeemHelperProportional(uint256 _amount, uint256 _balanceUnderlying) private returns (uint256 redeemedTokens) {
    address currToken;
