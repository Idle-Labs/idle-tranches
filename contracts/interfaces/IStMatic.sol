...
interface IStMATIC is IERC20Detailed {
  ...
  /// @notice Returns the total amount of stake the StMatic contract has in a validator
  // function getTotalStake(IValidatorShare _validatorShare) external view returns (uint256, uint256);

  /// @notice Returns the reward accumulated by the StMATIC contract in a specific validator share.
  // function getLiquidRewards(IValidatorShare _validatorShare) external view returns (uint256);
