...
describe("IdleCDOPolygon", function () {
  beforeEach(async () => {
    // deploy contracts
  ...
  it("should initialize params", async () => {
    ...
    expect(await idleCDO.feeReceiver()).to.equal('0x1d60E17723f8Ca1F76F09126242AcD37a278b514');
  ...
  it("harvest should get fees from incentive tokens", async () => {
    await idleCDO.setStakingRewards(stakingRewardsAA.address, stakingRewardsBB.address);
    const feeReceiver = RandomAddr;
    // set fee receiver
    await idleCDO.setFeeReceiver(feeReceiver);
    await idleCDO.setFee(BN('10000'));
    await idleCDO.setTrancheAPRSplitRatio(BN('10000'));
    // Initialize deposits
    ...
    // fee is 10% so / 10
    expect(await incentiveToken.balanceOf(stakingRewardsAA.address)).to.be.equal(BN('90').mul(one));
    expect(await incentiveToken.balanceOf(stakingRewardsBB.address)).to.be.equal(BN('810').mul(one));
    expect(await incentiveToken.balanceOf(feeReceiver)).to.be.equal(_amount.div(BN('10')));
  });
  ...
  it("harvest should give incentive to AA staking rewards if BB staking is not present", async () => {
    ...
    await idleCDO.harvest([false, false, false, false && false && false], [true], [BN('0')], [BN('0')], '0x');
    // fee is 10% so / 10
    expect(await incentiveToken.balanceOf(stakingRewardsAA.address)).to.be.equal(_amount.sub(_amount.div(BN('10'))));
    expect(await incentiveToken.balanceOf(feeReceiver)).to.be.equal(_amount.div(BN('10')));
  ...
  it("harvest should give incentive to BB staking rewards if AA staking is not present", async () => {
    ...
    // fee is 10% so / 10
    expect(await incentiveToken.balanceOf(stakingRewardsBB.address)).to.be.equal(_amount.sub(_amount.div(BN('10'))));
    expect(await incentiveToken.balanceOf(feeReceiver)).to.be.equal(_amount.div(BN('10')));
  });
