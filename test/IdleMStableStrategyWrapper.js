...
describe.only("IdleMStable Strategy Wrapper", function () {
  ...
  beforeEach(async () => {
    ...
    await DAI.connect(dai_signer).transfer(AABuyer.address, AMOUNT_TO_TRANSFER.mul(10));
    await DAI.connect(dai_signer).transfer(BBBuyer.address, AMOUNT_TO_TRANSFER.mul(10));
    ...
    let idleMStableStrategyWrapperFactory = await ethers.getContractFactory("IdleMStableStrategyWrapper");
    idleMStableStrategyWrapper = await idleMStableStrategyWrapperFactory.deploy(mUSD_ADDRESS, idleCDO.address);
  });

  it("Deposit with with AA token", async () => {
    let AATrancheTokenAddress = await idleCDO.AATranche();
    let aaTokenContract = await ethers.getContractAt(erc20.abi, AATrancheTokenAddress);
    let aaTokenBalanceBefore = await aaTokenContract.balanceOf(AABuyer.address);
    await DAI.connect(AABuyer).approve(idleMStableStrategyWrapper.address, AMOUNT_TO_TEST);
    await idleMStableStrategyWrapper.connect(AABuyer).depositAAWithToken(DAI.address, AMOUNT_TO_TEST, "1");
  ...
  it("Deposit with BB Token", async () => {
    ...
    let bbTokenContract = await ethers.getContractAt(erc20.abi, BBTrancheTokenAddress);
    let bbTokenBalanceBefore = await bbTokenContract.balanceOf(BBBuyer.address);
    await DAI.connect(BBBuyer).approve(idleMStableStrategyWrapper.address, AMOUNT_TO_TEST);
    await idleMStableStrategyWrapper.connect(BBBuyer).depositBBWithToken(DAI.address, AMOUNT_TO_TEST, "1");
  ...
  it("Withdraw AA Token", async () => {
    ...
    await aaTokenContract.connect(AABuyer).approve(idleMStableStrategyWrapper.address, aaTokensReceived);
    await idleMStableStrategyWrapper.connect(AABuyer).withdrawTokenViaBurningAA(DAI.address, aaTokensReceived, "1");
  ...
  it("Withdraw BB Token", async () => {
    ...
    await bbTokenContract.connect(BBBuyer).approve(idleMStableStrategyWrapper.address, bbTokensReceived);
    await idleMStableStrategyWrapper.connect(BBBuyer).withdrawTokenViaBurningBB(DAI.address, bbTokensReceived, "1");
