...
describe("ConvexStrategyETH (using stETH pool for tests)", async () => {
  ...
  beforeEach(async () => {
    strategy = await helpers.deployUpgradableContract('ConvexStrategyETH', [POOL_ID_STECRV, WETH, DEPOSIT_POSITION_STECRV, owner.address, [reward_crv, reward_cvx], weth2deposit]);
  });
