...
contract MetamorphoIdleVault is Test {
  // @notice test tranche prices pre and post redeem
  uint256 BLOCK = 19434222;
  ...
  function setUp() public {
    ...
    IERC20Detailed(WETH).transfer(DEPLOYER, amount);

    // Deposit WETH in AA_re7WETH with DEPLOYER
  ...
  function testMetamorphoFactoryCreation() external {
    ...
    vm.startPrank(DEPLOYER);
    // approve morpho to spend loanToken and collateralToken
    ...
    params[0] = re7Params;
    address mmVault = _createMetaMorphoVault(WETH, params);
  ...
  function _createOracle(TranchesChainlinkOracle adapter, uint256 collateralDecimals, uint256 loanDecimals) internal asUser(DEPLOYER) returns (IMorphoChainlinkOracleV2 oracle) {
    oracle = ORACLE_FACTORY.createMorphoChainlinkOracleV2(
      ...
      IAggregatorV3Minimal(address(0)), 
      IAggregatorV3Minimal(address(0)), 
      loanDecimals, // loan token decimals 
      '0x'
    );
    return oracle;
  }

  function _initializeTrancheMarket(address collateralToken) internal asUser(DEPLOYER) returns (IMorpho.MarketParams memory marketParams, bytes32 id) {
  ...
  function _createMetaMorphoVault(address loanToken, IMorpho.MarketParams[] memory params) internal asUser(DEPLOYER) returns (address mmVault) {
